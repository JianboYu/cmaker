...
#ifndef PROTOCOL_METRICS_H_
#define PROTOCOL_METRICS_H_

#include <string>

#include "os_assert.h"
#include "protocol_common_types.h"
#include "protocol_atomicops.h"
...
//   Histogram* protocol::metrics::HistogramFactoryGetCounts(
//       const std::string& name, int sample, int min, int max,
//       int bucket_count);
//   Histogram* protocol::metrics::HistogramFactoryGetEnumeration(
//       const std::string& name, int sample, int boundary);
//   void protocol::metrics::HistogramAdd(
...
#define RTC_HISTOGRAM_COUNTS(name, sample, min, max, bucket_count) \
  RTC_HISTOGRAM_COMMON_BLOCK(name, sample, false, \
      protocol::metrics::HistogramFactoryGetCounts(name, min, max, bucket_count))
...
#define RTC_HISTOGRAM_ENUMERATION(name, sample, boundary) \
  RTC_HISTOGRAM_COMMON_BLOCK(name, sample, false, \
      protocol::metrics::HistogramFactoryGetEnumeration(name, boundary))
...
#define RTC_HISTOGRAM_COMMON_BLOCK(constant_name, sample, log, \
  ...
  do { \
    ...
    static protocol::metrics::Histogram* atomic_histogram_pointer = nullptr; \
    protocol::metrics::Histogram* histogram_pointer = \
        AtomicOps::AcquireLoadPtr(&atomic_histogram_pointer); \
    if (!histogram_pointer) { \
      histogram_pointer = factory_get_invocation; \
      protocol::metrics::Histogram* prev_pointer = \
          AtomicOps::CompareAndSwapPtr( \
              &atomic_histogram_pointer, \
              static_cast<protocol::metrics::Histogram*>(nullptr), \
              histogram_pointer); \
        CHECK(prev_pointer || \
                 prev_pointer == histogram_pointer); \
    } \
    protocol::metrics::HistogramAdd(histogram_pointer, constant_name, sample); \
  } while (0)
...
#define RTC_HISTOGRAM_COMMON_BLOCK_SLOW(name, sample, factory_get_invocation) \
  do { \
    protocol::metrics::Histogram* histogram_pointer = factory_get_invocation; \
    protocol::metrics::HistogramAdd(histogram_pointer, name, sample); \
  } while (0)
