...
using protocol::RTCPUtility::RtcpCommonHeader;
...
bool App::Parse(const RtcpCommonHeader& header, const uint8_t* payload) {
  CHECK_EQ(header.packet_type, (uint8_t)kPacketType);
...
void App::WithData(const uint8_t* data, size_t data_length) {
  ...
  CHECK_LE(data_length, (size_t)kMaxDataSize);/* << "App data size << " << data_length
                                          << "exceed maximum of "
                                          << kMaxDataSize << " bytes.";*/
