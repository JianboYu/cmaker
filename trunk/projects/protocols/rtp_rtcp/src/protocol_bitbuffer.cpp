...
uint8_t WritePartialByte(uint8_t source,
  ...
  CHECK_LT((uint32_t)target_bit_offset, 8U);
  CHECK_LT((uint32_t)source_bit_count, 9U);
  CHECK_LE((uint32_t)source_bit_count, (uint32_t)(8 - target_bit_offset));
...
BitBuffer::BitBuffer(const uint8_t* bytes, size_t byte_count)
  ...
  CHECK_LE(static_cast<uint64_t>(byte_count_),
...
bool BitBuffer::ReadUInt8(uint8_t* val) {
  ...
  CHECK_LE(bit_val, std::numeric_limits<uint8_t>::max());
...
bool BitBuffer::ReadUInt16(uint16_t* val) {
  ...
  CHECK_LE(bit_val, std::numeric_limits<uint16_t>::max());
...
void BitBuffer::GetCurrentOffset(
  ...
  CHECK(out_byte_offset);
  CHECK(out_bit_offset);
