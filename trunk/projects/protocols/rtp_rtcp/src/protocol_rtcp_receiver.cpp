...
RTCPReceiver::RTCPReceiver(
  ...
  _criticalSectionRTCPReceiver.reset(Mutex::Create());
  _criticalSectionFeedbacks.reset(Mutex::Create());
}
...
int64_t RTCPReceiver::LastReceived() {
  AutoLock cs(_criticalSectionRTCPReceiver.get());
...
void RTCPReceiver::HandleSDESChunk(RTCPUtility::RTCPParserV2& rtcpParser) {
  ...
  {
    AutoLock cs(_criticalSectionFeedbacks.get());
