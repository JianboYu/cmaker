...
#ifndef PROTOCOL_EVENT_LOG_H_
#define PROTOCOL_EVENT_LOG_H_

#include <memory>
#include <string>


#include "protocol_platform_file.h"
...
class RtcEventLog {
 public:
...
#if 0
  // Logs configuration information for protocol::VideoReceiveStream.
  virtual void LogVideoReceiveStreamConfig(
      const VideoReceiveStream::Config& config) = 0;

  // Logs configuration information for protocol::VideoSendStream.
  ...
  static bool ParseRtcEventLog(const std::string& file_name,
                               protocol_log::EventStream* result);
};

}  // namespace protocol

#endif  // PROTOCOL_EVENT_LOG_H_
